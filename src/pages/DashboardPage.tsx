    ...
    <div className="min-h-screen bg-[#f3f3f3] text-sm font-sans">
      ...
      <div className="bg-[#2d3e50] text-white p-3 flex justify-between items-center border-b-4 border-yellow-500">
        <div className="flex items-center space-x-4">
          <div className="bg-yellow-500 text-[#2d3e50] font-black px-2 py-1 italic">SIAK<span className="text-white">NG</span>-LITE</div>
          <div className="hidden md:block text-[10px] uppercase tracking-tighter">Sistem Informasi Akademik</div>
        </div>
        <div className="flex items-center space-x-4 text-[11px] uppercase font-bold">
          <span>{new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
          <button 
            onClick={handleLogout}
            className="bg-red-600 hover:bg-red-700 px-3 py-1 border border-red-800 active:translate-y-px transition-all"
          >
            Logout
          </button>
        </div>
